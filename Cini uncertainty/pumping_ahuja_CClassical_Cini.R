#simulate ahuja C experiment using hydrus template files
...
deptha=c(-0.12,-0.24,-0.74,-1.5,-2.5,-3.5,-4.5)
conca=c(0.002152, 0.002636,0.002959,0.003692,0.00414,0.004391,0.004433)

#name of parameter file
para.filename='parameters.csv'

#name of hydrus folder with reference files to copy
hydrus.filename='ahuja_clay'
...
#read Cini
Cini=read.csv(here("CiniCSampled.csv"))
...
write.table(header1,here('Ciniclass_resultC.csv'),sep=',',col.names = F)


header2=data.frame(paste("P",1:8,sep=''))
write.table(header2,here('Cini_classpointC.csv'),sep=',',col.names = F)
...
for (i in 1:nrow(Cini)){
  ...
  table=cbind(time,rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n),round(htop*100,4),rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n))
	...
	lines.to.write2=c("Pcp_File_Version=4"
	                  ...
	                  ,"      0          0"
	                  ,"   thr     ths    Alfa      n         Ks       l"
	                  ,stri_join(c(0.012,0.55,0.119,0.899,0.0144,0.5),collapse='   ')
	                  ...
	                  , stri_join(c(1.21,956,1,0),collapse='   ')
	...
	table.prof=cbind(seq(1,1001,1),seq(-0,-20,-0.02),rep(0.55,1001),rep(1,1001),rep(1,1001),rep(0,1001),rep(1,1001),
...
# ***************************************************************
  ...
  write.table(data.frame(Cini[i,1],mass[i],mass2cm[i],mass.ini[i],mass2cm.ini[i],mass.ini[i]-mass[i],mass2cm.ini[i]-mass2cm[i],NSE[i]),
              here('Ciniclass_resultC.csv'),append = TRUE,col.names=F,sep=',')
  
  
  write.table(data.frame(Cini[i,1],t(conc.m)),here('Cini_classpointC.csv'),append = TRUE,col.names=F,sep=',')
