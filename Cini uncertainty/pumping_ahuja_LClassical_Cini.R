...
#read Cini
Cini=read.csv(here("CiniLSampled.csv"))
...
write.table(header1,here('Ciniclass_resultL.csv'),sep=',',col.names = F)


header2=data.frame(paste("P",1:8,sep=''))
write.table(header2,here('Cini_classpointL.csv'),sep=',',col.names = F)
...
for (i in 1:nrow(Cini)){
  ...
  table=cbind(time,rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n),round(htop*100,4),rep(0,n),rep(0,n),rep(0,n),rep(0,n),rep(0,n))
	...
	lines.to.write2=c("Pcp_File_Version=4"
	                  ...
	                  ,"      0          0"
	                  ,"   thr     ths    Alfa      n         Ks       l"
	                  ,stri_join(c(0.026,0.49,0.176,0.873,0.0340,0.5),collapse='   ')
	                  ...
	                  , stri_join(c(1.42,274,1,0),collapse='   ')
	...
	table.prof=cbind(seq(1,1001,1),seq(-0,-20,-0.02),rep(0.49,1001),rep(1,1001),rep(1,1001),rep(0,1001),rep(1,1001),
...
# ***************************************************************
  ...
  write.table(data.frame(Cini[i,1],mass[i],mass2cm[i],mass.ini[i],mass2cm.ini[i],mass.ini[i]-mass[i],mass2cm.ini[i]-mass2cm[i],NSE[i]),
              here('Ciniclass_resultL.csv'),append = TRUE,col.names=F,sep=',')
  
  
  write.table(data.frame(Cini[i,1],t(conc.m)),here('Cini_classpointL.csv'),append = TRUE,col.names=F,sep=',')
